...
class RackDevice(Interface):
    @staticmethod
    def serialize_device(rackdevice):
        if rackdevice is None:
            return None

        device_id = Interface.save_obj(rackdevice)
        return {
            "id": rackdevice,
            "name": rackdevice.name,
            "type": str(rackdevice.type),
            "class_name": rackdevice.class_name,
            "can_have_chains": rackdevice.can_have_chains,
            "chains": rackdevice.chains
        }
